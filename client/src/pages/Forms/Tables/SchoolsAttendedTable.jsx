...
const SchoolsAttendedTable = () => {
  const { formData, setFormData } = useUserInfo();

  // Ensure university and entries exist
  const entries = formData.schools_Attended?.entries || [];
  ...
  return (
    <div className="overflow-x-auto mb-4">
      <h3 className="font-semibold mb-2">Schools Attendant</h3>
      <table className="table-auto w-full border mb-2">
        <thead>
          <tr className="bg-gray-200">
            <th className="px-2 py-1 border">Name of School</th>
        ...
        <tbody>
          {entries.map((row, index) => (
            <tr key={index}>
              {['name_of_school', 'from', 'to'].map((field) => (
...
export default SchoolsAttendedTable;
