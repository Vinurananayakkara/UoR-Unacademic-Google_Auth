...
// Define fields and input types for each section
const SECTIONS = {
  professional: [
    { key: 'institute', label: 'Institute', type: 'text' },
    { key: 'course', label: 'Course', type: 'text' }, // <-- match UserInfoPage.jsx
...
const createEmptyOtherRow = (fields) =>
  Object.fromEntries(fields.map(f => [f.key, '']));

const createEmptySubjectRow = () => ({ subject: '', grade: '' });

const DynamicTable = ({ section, rows, setRows }) => {
  const fields = SECTIONS[section];
   const { formData, setFormData } = useUserInfo(); // <-- Add this
  ...
  const handleChange = (rowIdx, key, value) => {
    ...
    setRows(updated);
    setFormData({ ...formData, [section]: updated }); // <-- Sync to context
  };

  const addOtherRow = () => {
    const updated = [...rows, createEmptyOtherRow(fields)];
    setRows(updated);
    setFormData({ ...formData, [section]: updated }); // <-- Sync to context
  };

  // ...rest of DynamicTable...

 

  return (
    <div className="mb-6">
      <h3 className="font-bold mb-2">{section.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}</h3>
      <table className="table-auto w-full border mb-2">
        <thead>
          <tr>
            {fields.map(f => (
              <th key={f.key} className="border border-black">{f.label}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i}>
              {fields.map(f => (
                <td key={f.key} className="border border-black">
                  <input
                    type={f.type}
                    value={row[f.key]}
...
// ...existing code...
  ...
  const [professional, setProfessional] = React.useState(formData.professional || [createEmptyOtherRow(SECTIONS.professional)]);
  const [other_education, setOtherEducation] = React.useState(formData.other_education || [createEmptyOtherRow(SECTIONS.other_education)]);
  const [other, setOther] = React.useState(formData.other || [createEmptyOtherRow(SECTIONS.other)]);
  const [sports, setSports] = React.useState(formData.sports || [createEmptyOtherRow(SECTIONS.sports)]);
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 px-4">
      <div className="bg-white p-6 rounded-xl shadow-md w-full max-w-4xl">
        ...
        <div>
          ...
          <DynamicTable section="professional" rows={professional} setRows={setProfessional} />
        </div>
        ...
        <div>
          ...
          <DynamicTable section="other_education" rows={other_education} setRows={setOtherEducation} />
        </div>

        <div>
          <label className="block font-medium mb-1">Other Qualifications</label>
          <DynamicTable section="other" rows={other} setRows={setOther} />
        </div>

        <div>
          <label className="block font-medium mb-1">Sports Qualifications</label>
          <DynamicTable section="sports" rows={sports} setRows={setSports} />
        </div>
